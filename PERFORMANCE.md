...
### End-to-end save timing boundaries

Successful editor saves now record `save.total` from save acceptance through the native invocation and scheduling the saved state. Failures record the same span with an error outcome. The existing `ipc.save_file` remains the invocation-only boundary. A separate `ui.save-ready` foreground measurement starts at acceptance and finishes two animation frames after the successful saved-state effect commits. It includes IPC and render scheduling; do not add these overlapping spans. Neither measure includes later watcher/status reconciliation. These are presentation opportunities rather than physical display timings.
...
### History paging and layout attribution

Accepted near-end page requests now record `history.page` from acceptance through the full history refresh and state-update scheduling. Failed requests and requests interrupted by a repository-generation change are error outcomes; ordinary scroll events rejected by the existing in-flight guard do not create page samples. This span overlaps `git.refresh.history` / `ipc.refresh_repository` and does not include the subsequent rendered checkpoint. The history component separately records `history.layout` around actual `layoutGraph` executions inside its existing memoization; this measures synchronous frontend layout computation, excluding React/DOM rendering and native Git work.

Paging currently increases the fetched prefix by 500 commits and uses a full repository snapshot, including status/file discovery. Changing that policy requires native cost and correctness evidence; these new boundaries make those costs distinguishable instead of treating a page load as one opaque delay. A deferred-request test verifies repeated near-end scroll events produce one in-flight request, no page duration appears before completion, and a failed request restores the old limit so the successful retry again requests 1,000 commits rather than skipping ahead. Full validation passes (135 frontend / 22 Rust tests). Native paging and render-attribution measurements remain outstanding.

## Stash selection (2026-09-18)

Read-only release benchmark: `cargo run --release --manifest-path src-tauri/Cargo.toml --example stash_performance -- /path/to/repository`. It compares the newest stash with its base commit over 12 runs without printing repository paths or source text.

On the primary development repository, median stash detail loading fell from **618.3 ms to 34.4 ms**; the regular commit comparison fell from **65.7 ms to 44.2 ms**. These are warm local backend timings, not complete UI or cold-cache measurements. The stash contained one untracked file; the comparison commit changed five files.

Metadata now uses `git log --no-walk -1` instead of `git show`, with one combined parents/message read. Tracked and untracked stash statistics run concurrently. Stash selection bypasses the 35 ms commit debounce, shares in-flight requests, and caches up to 64 immutable detail results by repository and stash OID. `stash.details` records uncached frontend detail request time in the existing performance report; diff preparation remains covered by the existing diff spans.
