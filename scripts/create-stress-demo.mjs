...
if (existsSync(root)) { console.log(`Stress repository already exists:\n${root}`); process.exit(0); }
mkdirSync(root);
...
for (let i = 1; i <= 6000; i++) {
  ...
  if (i === 1) {
    for (let f = 0; f < 10000; f++) stream += `M 100644 inline packages/package-${Math.floor(f/100)}/src/file-${f}.ts\n` + data(`export const sample${f} = ${f};\n`);
  }
  stream += 'M 100644 inline sequence.ts\n' + data(`// Synthetic performance workload\nexport const sequence = ${i};\n`) + '\n';
}
execFileSync('git', ['fast-import', '--quiet'], { cwd: root, input: stream, stdio: ['pipe', 'pipe', 'pipe'] });
// Populate a newly created fixture's worktree only.
git('reset', '--hard', 'HEAD');
for (let i = 1; i <= 12; i++) git('update-ref', `refs/remotes/origin/sample-${i}`, git('rev-parse', `HEAD~${i*100}`).trim());
